...
template <typename Key, typename Tp, std::size_t N, bool Fix>
using map_3 = std::map<Key, Tp, std::less<Key>, custom::AllocatorClassic<std::pair<const Key, Tp>, N, Fix>>;
...
int main(int argc, char const* argv []) {
	...
	// создание экземпляра std::map<int, int> с новым аллокатором, ограниченным 10 элементами
	map_3<int, int, num_elements, fix_my_mem> factorials_with_cust_classic_alloc;
	// заполнение 10 элементами, где ключ - это число от 0 до 9, а значение - факториал ключа
	fill_map(factorials_with_cust_classic_alloc);

	// вывод на экран всех значений (ключ и значение разделены пробелом) хранящихся в контейнере
	print_map(factorials);
	// print_map(factorials_with_cust_alloc);
	print_map(factorials_with_cust_classic_alloc);
	...
	// создание экземпляра std::vector<int> с новым аллокатором, ограниченным 10 элементами
	vct_3<int, num_elements, fix_my_mem> seq_with_cust_classic_alloc;
	// заполнение 10 элементами от 0 до 9
	fill_vector(seq_with_cust_classic_alloc);

	// вывод на экран всех значений, хранящихся в контейнере
	print_vector(seq);
	// print_vector(seq_with_cust_alloc);
	print_vector(seq_with_cust_classic_alloc);
